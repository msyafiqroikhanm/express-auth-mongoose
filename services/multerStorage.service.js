    ...
    filename: (_req, _file, cb) => {
        if (target) {
            if (target === 'committee') {
                ...
                cb(
                    null,
                    `${target}-${slugedName}-${slugedPhoneNbr}-${Date.now()}${path.extname(file.originalname).toLowerCase()}`
                );
            } else {
                cb(
                    null,
                    `${target}-${Date.now()}${path.extname(file.originalname).toLowerCase()}`
                );
            }
        } else {
            cb(
...
const participantStorage = multer.diskStorage({
    ...
    filename: (_req, _file, cb) => {
        if (target) {
            ...
            cb(
                null,
                `${slugedName}-${slugedPhoneNbr}-${Date.now()}${path.extname(file.originalname).toLowerCase()}`
            );
        }
    },
});
